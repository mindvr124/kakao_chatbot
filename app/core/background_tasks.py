    ...
    try:
        ...
        async for session in get_session():
            ...
            try:
                ...
                try:
                    await save_log_message(session, "message_saved_user", user_id, conv_id, request_id, f"content_len_{len(user_text)}")
                except Exception:
                    pass
            except Exception:
                try:
    ...
    try:
        ...
        async for session in get_session():
            ...
            try:
                ...
                try:
                    await save_log_message(session, "message_saved_assistant", user_id, conv_id, request_id, f"tokens_{tokens_used}")
                except Exception:
                    pass
            except Exception:
                try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        await save_log_message(session, "summary_saved", user_id, conv_uuid, None, f"len_{len(summary_text or '')}")
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        await upsert_user_summary_from_text(session, user_id, summary_text)
                        try:
                            await save_log_message(session, "summary_saved", user_id, conv_uuid, None, f"len_{len(summary_text or '')}_after_rollback")
                        except Exception:
                            pass
                    except Exception:
                        try:
                            await save_log_message(session, "summary_failed", user_id, conv_uuid, None, f"error_{str(err)[:300]}")
                        except Exception:
                            pass
                        raise
                ...
                try:
                    await save_log_message(session, "summary_failed", user_id, conv_uuid, None, f"error_{str(e)[:300]}")
                except Exception:
                    pass
